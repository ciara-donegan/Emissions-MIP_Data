...
for(scenario in scenarios){
  ...
  experiment <- bind_rows(map(target_filename, read.csv, stringsAsFactors = FALSE))
  ...
  # Correct model names
  if (scenario %in% c('shp-10p-red', 'shp-10p-red-1950', 'shp-20p-red', 'shp-20p-red-1950',
                  'shp-80p-red')) {
  ...
  } else if (scenario %in% c('shp-atl-shift', 'shp-atl-shift-1950', 'shp-ind-shift',
                         'shp-ind-shift-1950')) {
    experiment$model[which(experiment$model == "CESM")] <- "CESM1"
  }
  ...
  if (sort_by == "region"){
    ...
    cltc_plot <- plot_species(loadbc_experiment, region, "load \n of bc", expression(loadbc~(kg~m^-2)))
